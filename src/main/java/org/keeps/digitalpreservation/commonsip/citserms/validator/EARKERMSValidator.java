...
import org.keeps.digitalpreservation.commonsip.citserms.validator.components.ermsControl.ermsValidator.ErmsValidatorComponentValidator210;
...
public class EARKERMSValidator {
   ...
   *
   * @param reportOutputJson the {@link ValidationReportOutputJson}
   * @throws IOException                  if some I/O error occurs.
   * @throws ParserConfigurationException if some error occurred.
   * @throws SAXException                 if some error occurred.
   */
   ...
   *
   * @throws IOException                  if some I/O error occurs.
   * @throws ParserConfigurationException if some error occur.
   * @throws SAXException                 if some error occur.
   */
   ...
   *
   * @param observer the {@link ValidationObserver}
   */
  ...
  public boolean validate(String version) throws IOException {
    ...
    /*if (validationReportOutputJson.validFileComponent()) {
      ...
      if (structureValidatorState.isZipFileFlag()) {
        ermsValidatorState.setErmsFiles(structureValidatorState.getZipManager().getFiles(earkermsPath));
        subMets = structureValidatorState.getZipManager().getSubMets(earkermsPath);
      } else {
        metsValidatorState.setMetsFiles(structureValidatorState.getFolderManager().getFiles(earkermsPath));
        subMets = structureValidatorState.getFolderManager().getSubMets(earkermsPath);
      }
   ...
   *
   * @param subMets the {@link Map } with path to sub METS and InputStream of file.
   * @param isZip   flag if the Information Package is in compact format or if it is a
   *                folder.
   */
  /*private void validateSubMets(final Map<String, InputStream> subMets, final boolean isZip) {
    for (Map.Entry<String, InputStream> entry : subMets.entrySet()) {
      ...
      try {
        ermsValidatorState.setErms(instatiateMets.instatiateMetsFile());
   ...
   * @param mets the path to METS file
   * @return a message
   */
  /*
  ...
  private void validateErms() {
    ...
    try {
      ...
      //     final ReporterDetails ermsStr0 = new ReporterDetails(Constants.VALIDATION_REPORT_HEADER_ERMS_VERSION, message,
      //       false, false);
   ...
   * Setup State of METS.
   *
   * @param key        the METS file path
   * @param isZip      Flag if Information package is compacted or not
   * @param isRootMets Flag if METS file is root or representation METS
   */
 /*
  ...
  */
  ...
  private List<ErmsValidator> getComponentsForVersion(String version) throws IOException, ParserConfigurationException, SAXException {
